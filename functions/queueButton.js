...
async function queueButton( client, interaction, pageNumber, updateMessage ) {
    ...
    if (!queue || !queue.node.isPlaying()){
        const musicEmbed = new EmbedBuilder().setColor(0xFF0000).setDescription(`**No Music in Queue!**`)
        if (updateMessage) {
    ...
    if (page >= totalPages) {
        const pageEmbed = new EmbedBuilder().setColor(0xFF0000).setTitle(`Invalid Page!`).setDescription(`there are only ${totalPages} pages`)
        if (updateMessage) {
